...
Linear::Linear(size_t in_features, size_t out_features)
    : weights(Tensor::random({in_features, out_features})),
      biases(Tensor::zeros({1, out_features})), last_input({0}) {}
...
Tensor Linear::backward(Tensor grad_output, float learning_rate) {
  ...
  Tensor grad_input =
      grad_output.matmul(weights_T); // [batch_size, in_features]

  // 2. Calculate grad_weights = last_input.T * grad_output
  Tensor input_T = last_input.transpose();
  Tensor grad_weights =
      input_T.matmul(grad_output); // [in_features, out_features]
  ...
  if (grad_output.device == Device::Metal) {
    ...
    // We lack a dedicated reduction sum kernel, so we'll pull grad_output to
    // CPU briefly to compute bias gradients. Bias gradients are small ([1,
    // out_features]).
    ...
    // For grad_biases, since we modified `data`, we must recreate the buffer or
    // `to(Metal)` won't copy if it thinks it's already there Actually,
    // `to(Metal)` only copies if `device != target_device`.
...
#ifdef YAN_USE_CUDA
  else if (weights.device == Device::CUDA) {
    lr_grad_weights.to(Device::CUDA);
    lr_grad_biases.to(Device::CUDA);
    weights = weights.subtract(lr_grad_weights);
    biases = biases.subtract(lr_grad_biases);
  }
#endif
...
// ReLU Activation
void ReLU::to(Device device) { last_input.to(device); }
...
Tensor ReLU::forward(const Tensor &input) {
  ...
  if (input.device == Device::Metal) {
    ...
    NS::UInteger threadGroupSizeX =
        ctx.reluPSO()->maxTotalThreadsPerThreadgroup();
    ...
    out.to(Device::CPU);
    out.to(Device::Metal); // Sync memory
    return out;
#else
    throw std::runtime_error(
        "Metal backend not available when compiled with CUDA.");
    ...
    out.to(Device::CUDA);
    cuda::launch_relu(static_cast<const float *>(input.cuda_buffer),
                      static_cast<float *>(out.cuda_buffer), input.data.size());
    cudaDeviceSynchronize();
    out.to(Device::CPU);
    out.to(Device::CUDA); // Sync memory
    return out;
  }
#endif
...
Tensor ReLU::backward(Tensor grad_output, float learning_rate) {
  ...
  if (grad_output.device == Device::Metal) {
    ...
    encoder->setBuffer(static_cast<MTL::Buffer *>(grad_output.mtl_buffer), 0,
                       0);
    ...
    NS::UInteger threadGroupSizeX =
        ctx.reluBackwardPSO()->maxTotalThreadsPerThreadgroup();
    MTL::Size threadgroupSize = MTL::Size::Make(threadGroupSizeX, 1, 1);
    ...
    grad_input.to(Device::CUDA);
    cuda::launch_relu_backward(
        static_cast<const float *>(grad_output.cuda_buffer),
        static_cast<const float *>(last_input.cuda_buffer),
        static_cast<float *>(grad_input.cuda_buffer), grad_output.data.size());
    cudaDeviceSynchronize();
    grad_input.to(Device::CPU);
    grad_input.to(Device::CUDA);
    return grad_input;
...
// Sigmoid Activation
void Sigmoid::to(Device device) { last_output.to(device); }
...
Tensor Sigmoid::forward(const Tensor &input) {
  ...
  if (input.device == Device::Metal) {
    ...
    MTL::Size gridSize = MTL::Size::Make(input.data.size(), 1, 1);
    NS::UInteger threadGroupSizeX =
        ctx.sigmoidPSO()->maxTotalThreadsPerThreadgroup();
    MTL::Size threadgroupSize = MTL::Size::Make(threadGroupSizeX, 1, 1);
    ...
    out.to(Device::CUDA);
    cuda::launch_sigmoid(static_cast<const float *>(input.cuda_buffer),
                         static_cast<float *>(out.cuda_buffer),
                         input.data.size());
    cudaDeviceSynchronize();
    out.to(Device::CPU);
    out.to(Device::CUDA);
...
Tensor Sigmoid::backward(Tensor grad_output, float learning_rate) {
  ...
  if (grad_output.device == Device::Metal) {
    ...
    encoder->setBuffer(static_cast<MTL::Buffer *>(grad_output.mtl_buffer), 0,
                       0);
    encoder->setBuffer(static_cast<MTL::Buffer *>(last_output.mtl_buffer), 0,
                       1);
    encoder->setBuffer(static_cast<MTL::Buffer *>(grad_input.mtl_buffer), 0, 2);

    MTL::Size gridSize = MTL::Size::Make(grad_output.data.size(), 1, 1);
    NS::UInteger threadGroupSizeX =
        ctx.sigmoidBackwardPSO()->maxTotalThreadsPerThreadgroup();
    MTL::Size threadgroupSize = MTL::Size::Make(threadGroupSizeX, 1, 1);
    ...
    grad_input.to(Device::CUDA);
    cuda::launch_sigmoid_backward(
        static_cast<const float *>(grad_output.cuda_buffer),
        static_cast<const float *>(last_output.cuda_buffer),
        static_cast<float *>(grad_input.cuda_buffer), grad_output.data.size());
    cudaDeviceSynchronize();
    grad_input.to(Device::CPU);
    grad_input.to(Device::CUDA);
